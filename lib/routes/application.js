...
const applicationRoute = (httpServer) => {
  ...
  const isPB = (msg) => {
    try {
      ...
      for (let value of Object.keys(message)) {
        let type = message[value].type;
        let id = parseInt(message[value].id);
        let name = message[value].name;
        let unit = message[value].unit;
      ...
      idArr.sort((a, b) => a - b);
      return idArr[0] === 1 && idArr[idArr.length - 1] === idArr.length;
    } catch (err) {
      console.log(err);
      return false;
    }
